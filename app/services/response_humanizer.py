...
async def humanize_response(
    *,
    ...
    system_prompt = (
        ...
        "2. Match their language exactly as per their language preference {language_mix} — Hinglish stays Hinglish, English stays English, Hindi stays Hindi\n"
        "3. CRITICAL: Do not switch languages mid-sentence. If Hinglish, keep ALL connective phrases in Hinglish too (use 'dekho', 'yeh raha', 'achha', NOT 'Let me show you', 'Here it is'). Consistency within each sentence.\n"
        "4. Sound like a helpful human coach, never a form or system notification\n"
        "5. If this is onboarding context, explain WHY you need the info — not just WHAT\n"
        "6. No AI-speak: no 'Here is', 'As an AI', 'In conclusion', 'Important note'\n"
        "7. Plain text only — no markdown, no formatting, no emojis unless the original had them\n"
        ...
        "- Target (IMPORTANT, CRITICAL): Keep response under 500 characters when possible. If you think message is exceeding 500 cahrs try thingking smartly how can you catorgirze them furether to bring it down to under 500 characters. If original has too much info, "
