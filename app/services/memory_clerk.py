...
async def ai_memory_clerk(
    ...
    system_prompt = (
        ...
        "- IMPORTANT: Distinguish between physical injuries and chronic medical conditions:\n"
        "  * Physical injuries (knee pain, shoulder injury, back pain, ankle sprain, muscle tear, etc.) "
        "→ return under physiology.injuries as array of objects with keys: part, severity, history, pain_triggers.\n"
        "  * Chronic medical conditions (diabetes, hypertension, PCOS, thyroid, asthma, heart condition, etc.) "
        "→ return under physiology.medical_flags as array of strings (e.g., ['diabetes', 'hypertension']).\n"
        "- If user mentions diabetes/BP/chronic condition, do NOT put it in injuries array.\n"
        "- If user says no injuries (none/no issue/fit), set physiology.injuries to empty array [].\n"
        ...
        "- If user explicitly requests language change (e.g., 'language change karo', "
        "'speak in English only', 'Hindi mein baat karo', 'switch to Hinglish'), "
        "set identity.language_mix to 'English', 'Hindi', or 'Hinglish' accordingly.\n"
        "- Do NOT change language_mix based on the language they're currently speaking in. "
        "Only change it when they explicitly ask to change language preference.\n"
        ...
        "- If user mentions both chronic conditions AND physical fitness (e.g., 'diabetes hai but body 100% fit'), "
        "set medical_flags appropriately and injuries to empty array. Mark 'injuries' as resolved.\n"
        "- When marking onboarding_fields_resolved: mark 'injuries' as resolved if user answered about physical injuries, "
        "even if they also have medical conditions (those go to medical_flags).\n"
